...
import "./interfaces/IObservableConfidentialLaunchpadMigrator.sol";
import "./ObservableConfidentialLaunchpadMigrator.sol";

/**
 * @title ObservableConfidentialLaunchInitializationStrategy
 ...
 */
contract ObservableConfidentialLaunchInitializationStrategy is
    IObservableConfidentialInitializationStrategy
{
    ...
    constructor(
        ...
        if (
            IObservableConfidentialCPMMFactory(factory_).PROTOCOL_VERSION() !=
                PROTOCOL_VERSION ||
            IObservableConfidentialCPMMFactory(factory_).PRIVACY_MODE() !=
                PRIVACY_MODE
        ...
        address deployedMigrator = address(
            new ObservableConfidentialLaunchpadMigrator(
                factory_,
                address(this)
            )
        );
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return
            interfaceId ==
                type(IObservableConfidentialInitializationStrategy).interfaceId ||
    ...
    function prepareLaunch(
        ...
        IObservableConfidentialCPMMFactory canonicalFactory =
            IObservableConfidentialCPMMFactory(factory);
        ...
        launches[launchId] = LaunchRecord({
            ...
            creator: creator,
            pool: pool,
            initialPriceReferenceX18: initialPriceReferenceX18,
    ...
    function authorizeInitialization(
        ...
        if (
            ...
            initialPriceReferenceX18 != record.initialPriceReferenceX18 ||
            IObservableConfidentialCPMM(pool).initialized() ||
            IObservableConfidentialCPMM(pool).initialPriceReferenceX18() != 0
