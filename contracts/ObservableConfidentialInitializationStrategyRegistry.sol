...
/**
 * @title ObservableConfidentialInitializationStrategyRegistry
 * @notice One-time reviewed strategy registry bound to one confidential factory.
 */
contract ObservableConfidentialInitializationStrategyRegistry is
    IObservableConfidentialInitializationStrategyRegistry
{
    ...
    function supportsInterface(bytes4 interfaceId) external pure returns (bool) {
        return
            interfaceId ==
            type(IObservableConfidentialInitializationStrategyRegistry).interfaceId ||
    ...
    function bindFactory(address factory_) external {
        ...
        if (
            ...
            IObservableConfidentialCPMMFactory(factory_)
    ...
    function registerInitializationStrategy(address strategy) external {
        ...
        IObservableConfidentialInitializationStrategy candidate =
        ...
        if (
            ...
            IObservableConfidentialLaunchpadMigrator(migrator)
                .PROTOCOL_VERSION() != 1 ||
            IObservableConfidentialLaunchpadMigrator(migrator).factory() != factory ||
            IObservableConfidentialLaunchpadMigrator(migrator)
