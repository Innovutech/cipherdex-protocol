 ...
 *
 * This is a separate opt-in disclosure mode. It does not alter mode-1 pools or
 * claim that public price observations make individual amounts impossible to
 * infer. It has not received an independent external audit.
 */
contract ObservableConfidentialCPMM is CipherDEXFeePolicy {
    ...
    uint256 public initialPriceReferenceX18;
    ...
    uint256 public publicPriceQuantumX18;
    uint64 public publicObservationSequence;
    ...
    error InitialPriceOutsideObservationRange();
    ...
    function initializeLPToken(address lpToken_) external {
        ...
        IObservableConfidentialCPMMFactory canonicalFactory =
            IObservableConfidentialCPMMFactory(msg.sender);
    ...
    function initializeLiquidity(
        ...
        uint256 initialPriceReferenceX18_,
        uint64 deadline
    ...
    function _addLiquidity(
        ...
        bool expectedInitialized,
        uint64 deadline
    ) internal returns (ctUint256 memory mintedShares) {
        _requireBeforeDeadline(deadline);
        if (initialized != expectedInitialized) revert UnexpectedInitializationState();
        bool initializing = !initialized;
        if (initializing) {
            if (initialPriceReferenceX18_ == 0) {
                revert InvalidInitialPriceReference();
            }
        } else if (initialPriceReferenceX18_ != 0) {
        ...
        if (initializing) {
            initialPriceReferenceX18 = initialPriceReferenceX18_;
            _initializePublicObservation(nextReserve0, nextReserve1);
        }
    ...
    function _bootstrapLiquidity(
        ...
        if (initialPriceReferenceX18_ == 0) revert InvalidInitialPriceReference();
        ...
        initialPriceReferenceX18 = initialPriceReferenceX18_;
        _initializePublicObservation(amount0, amount1);
    ...
    function removeLiquidity(
        ...
        if (isFullExit) {
            ...
            initialized = false;
            delete initialPriceReferenceX18;
            _clearCurrentObservation();
        }
        reserve0State = MpcCore.offBoard(nextReserve0);
        reserve1State = MpcCore.offBoard(nextReserve1);
    ...
    function _initializePublicObservation(
        ...
        gtUint256 priceX18 = _normalizedPriceX18(reserve0, reserve1);
        uint256 referencePrice = initialPriceReferenceX18;
        uint256 lower = _referenceLower(referencePrice);
        uint256 upper = _referenceUpper(referencePrice);
        if (!MpcCore.decrypt(MpcCore.ge(priceX18, lower))) {
            revert InitialPriceOutsideObservationRange();
        }
        if (!MpcCore.decrypt(MpcCore.le(priceX18, upper))) {
            revert InitialPriceOutsideObservationRange();
        }
    ...
    function _publishObservation(
        ...
        if (bucket == 0 || quantum == 0) revert InvalidInitialPriceReference();
        uint64 sequence = publicObservationSequence + 1;
        publicObservationSequence = sequence;
        publicPriceBucketX18 = bucket;
        publicPriceQuantumX18 = quantum;
        publicObservationAt = observedAt;
        publicObservationPublishedAt = uint64(block.timestamp);
        publicObservationActivityCount = activityCount;
        emit PublicPriceObservation(
            sequence,
            bucket,
            observedAt,
    ...
    function _quantizePrice(
        gtUint256 priceX18,
        uint256 quantum
    ) internal returns (gtUint256) {
        gtUint256 bucketIndex = MpcCore.div(priceX18, quantum);
        return MpcCore.mul(bucketIndex, quantum);
    }

    function _priceQuantum(
        uint256 referencePrice
    ) internal pure returns (uint256) {
        uint256 quotient = referencePrice / FEE_DENOMINATOR;
        uint256 remainder = referencePrice % FEE_DENOMINATOR;
        uint256 quantum =
            quotient * OBSERVATION_BUCKET_BPS +
            (remainder * OBSERVATION_BUCKET_BPS) /
            FEE_DENOMINATOR;
        return quantum == 0 ? 1 : quantum;
    }

    function _referenceLower(
        uint256 referencePrice
    ) internal pure returns (uint256) {
        uint256 lower =
            referencePrice / OBSERVATION_REFERENCE_LOWER_DIVISOR;
        return lower == 0 ? 1 : lower;
    }

    function _referenceUpper(
        uint256 referencePrice
    ) internal pure returns (uint256) {
        if (
            referencePrice >
            type(uint256).max / OBSERVATION_REFERENCE_UPPER_MULTIPLIER
        ) return type(uint256).max;
        return referencePrice * OBSERVATION_REFERENCE_UPPER_MULTIPLIER;
    }
    ...
    function _requireBestExecutionRouter() internal view {
        if (
            ...
            IObservableConfidentialCPMMFactory(bootstrapper)
                .bestExecutionRouter() != msg.sender
