...
import {
  PUBLIC_STACK_MAX_GAS_UNITS,
} from "./deploy-public-stack";
import {
...
async function main(): Promise<void> {
  if (process.env.COTI_TESTNET_PRIVATE_KEY?.trim()) {
    throw new Error("mainnet public preflight refuses COTI_TESTNET_PRIVATE_KEY");
  }
  ...
  const recoveryKey = process.env.CIPHERDEX_DEPLOYMENT_RECOVERY_KEY?.trim();
  if (!recoveryKey || getBytes(recoveryKey).length !== 32) {
  ...
  requiredAddress("CIPHERDEX_FEE_BENEFICIARY");
  const wrappedNative = requiredAddress("CIPHERDEX_EXISTING_WRAPPED_NATIVE");
  await verifyDeployedRuntimeArtifactWithProvenance("WrappedNativeToken", wrappedNative);
  ...
  if (Boolean(configuredLedgerAddress) === Boolean(privateKey)) {
    throw new Error(
  ...
  if (configuredLedgerAddress) {
    ...
    deployerAddress = new Wallet(privateKey!).address;
  ...
  const maximumCost = PUBLIC_STACK_MAX_GAS_UNITS * REVIEWED_MAX_FEE_PER_GAS_WEI;
  if (balance < maximumCost) {
    throw new Error("mainnet deployer has insufficient native COTI for the public-stack gas caps");
  }

  console.log(
    `COTI mainnet public-stack preflight passed for ${deployerAddress}; ` +
      `commit=${sourceCommit}; wrappedNative=${wrappedNative}; ` +
      `signer=${signerSummary}; no transaction was signed or sent`,
  );
}

void main().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : "unknown failure";
  console.error(`COTI mainnet public-stack preflight failed: ${message}`);
  process.exitCode = 1;
});
